        ...
        execute: function () {///<reference path="../../../headers/common.d.ts" />
            TableRenderer = (function () {
                    ...
                    if (style.type === 'link') {
                        return function (v) {
                            ...
                            if (lodash_1.default.isString(v)) {
                                var theValue = v;
                                var theString = style.linkFormat;
                                // "gm" = RegEx options for Global search (more than one instance)
                                // and for Multiline search
                                var regEx = new RegExp("\\{" + 0 + "\\}", "gm");
                                return '<a ' + theString.replace(regEx, theValue) + '</a>';
                            }
                            return undefined;
                        };
                    }
                ...
                TableRenderer.prototype.renderCell = function (columnIndex, value, addWidthHack) {
                    ...
                    if(value.indexOf(',') != -1) {
                    	var colval= value.split(',');
                    	var output = '<table cellspacing="0" cellpadding="0" style="border-collapse: collapse;">';
                    	for(var colvalIndex = 0; colvalIndex < colval.length; colvalIndex++) {
                    		output = output + '<tr><td align="left" style="padding:0; margin:0;">' + colval[colvalIndex] + '</td></tr>';
                    	}
                    	output = output + '</table>';
                    	return '<td' + style + '>' + output + widthHack + '</td>';
                    } else {
